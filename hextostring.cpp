...
int hexToString::strHexToDecimal(const QString &strHex)
{
    bool ok;
    return strHex.toInt(&ok, 16);
}

QString hexToString::decimalToStrHex(int decimal)
{
...
QString hexToString::byteArrayToAsciiStr(const QByteArray &data)
{
    QString temp;
    int len = data.size();
...
QByteArray hexToString::hexStrToByteArray(const QString &str)
{
    QByteArray senddata;
...
char hexToString::convertHexChar(char ch)
{
    if ((ch >= '0') && (ch <= '9')) {
        return ch - 0x30;
    } else if ((ch >= 'A') && (ch <= 'F')) {
...
QByteArray hexToString::asciiStrToByteArray(const QString &str)
{
    QByteArray buffer;
    int len = str.length();
    QString letter;
...
QString hexToString::byteArrayToHexStr(const QByteArray &data)
{
    QString temp = "";
